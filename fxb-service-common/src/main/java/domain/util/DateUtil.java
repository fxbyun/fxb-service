...
public class DateUtil {
    ...
    */
    ...
    public static Boolean isAfterDayByZdCa(Calendar zdCalendar) {
        if (zdCalendar != null) {
            ...
            if (currentDate.after(zdDate)) {
                // 判断是否为同一天
                boolean is = org.apache.commons.lang.time.DateUtils.isSameDay(currentDate, zdDate);
                if (!is) {
                    return true;
                }
            }
     ...
     * @return
     */
    public static Integer calcCompareDate3(Date date1, Date date2) {
        try {
            if (date1.getTime() > date2.getTime()) {
                return 1;
            } else if (date1.getTime() < date2.getTime()) {
    ...
    /* 将当前日期加减n分数。
     * 如传入整型-3 意为将当前日期减去3分的日期
     * 如传入整型3 意为将当前日期加上3分后的日期
     */
     public static Calendar addMinute(int minute) {
         Calendar canlendar = Calendar.getInstance(); //java.util包
         canlendar.add(Calendar.MINUTE, minute); //日期减 如果不够减会将月变动
         return canlendar;
     }
     /* 将当前日期加减n小时数。
      * 如传入整型-3 意为将当前日期减去3小时的日期
      * 如传入整型3 意为将当前日期加上3小时后的日期
      */
      public static Calendar addHour(int hour) {
          Calendar canlendar = Calendar.getInstance(); //java.util包
          canlendar.add(Calendar.HOUR_OF_DAY, hour); //日期减 如果不够减会将月变动
          return canlendar;
      }
    /**
    ...
    public static void main(String[] args) throws Exception {
        ...
        Calendar  channelPrintEndTime = DateUtil.add(
        		DateUtil.parseYYYY_MM_DD_HH_MM_SS("2018-03-08 14:35:00"),
				Calendar.SECOND, -(50));
        System.out.println(getDateString2(channelPrintEndTime.getTime()));
	...
	public static Date dateFormatWeek(Date pushDateTime) {
		LocalDate local = LocalDate.now();
		LocalDate local0 = local.with(DayOfWeek.MONDAY);
		System.out.println(local0.toString());
		return null;
	}
	
	public static int getCurrentTimeWithTodayEndScends() {
		return (int) (DateUtil.getTodayEndTime().getTime() - new Date().getTime()) / 1000;
	}
	
	public static Date getDateTimeByDateStr(String dateStr) {
		Date date = new Date();
	    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		return getDateByString(sdf.format(date) + " " + dateStr);
	}
    ...
    public static boolean isBetweenRangTime(Date dateTime,String startTime, String endTime) {
		Date date00 = DateUtil.getDateTimeByDateStr(startTime);//"00:00:00"
		Date date02 = DateUtil.getDateTimeByDateStr(endTime);//"02:00:00"
		if(dateTime.compareTo(date00) == 0 || dateTime.compareTo(date02) == 0) {//包括边界值
			return true;
		}
		if(dateTime.after(date00) && dateTime.before(date02)) { //在 00点 至 02点 之间
			return true;
		}
		return false;
	}
